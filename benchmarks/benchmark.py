    ...
    while not stop_event.is_set():
        ...
        try:
            if random.random() < read_cutoff:
                ...
                # Range Query on Salary (Classic B-Tree candidate)
                if r < 0.25:
                    low = random.randint(30000, 120000)
                    cur.execute(
                        f"SELECT count(*) FROM employees WHERE salary BETWEEN {low} AND {low + 5000}"
                    )
                    target = "salary_range"

                # Point Lookup on Employee ID (Unique/High Cardinality)
                elif r < 0.50:
                    eid = random.randint(100000, 900000)
                    cur.execute(f"SELECT name FROM employees WHERE employee_id = {eid}")
                    target = "empid_point"

                # Text Pattern Search (LIKE 'User_100%') - Tests text indexing
                elif r < 0.75:
                    # Generate a prefix that matches some rows
                    prefix = f"User_{random.randint(1, 20000)}"
                    cur.execute(
                        f"SELECT count(*) FROM employees WHERE name LIKE '{prefix}%'"
                    )
                    target = "name_pattern"

                # Filter on Low Cardinality Column (Department)
                # Actuator should ideally NOT index this
                else:
                    dept = random.choice(
                        ["HR", "IT", "Sales", "Marketing", "Engineering"]
                    )
                    cur.execute(
                ...
                if r < 0.7:
                    cur.execute(
                        f"INSERT INTO employees (employee_id, name, salary, department, status) VALUES ({random.randint(900000, 999999)}, 'BenchUser', {random.randint(30000, 90000)}, 'Test', 'Active')"
                    )
                    target = "insert"
                # Update
                elif r < 0.9:
                    eid = random.randint(100000, 900000)
                    cur.execute(
                        f"UPDATE employees SET salary = salary + 50 WHERE employee_id = {eid}"
                    )
                    target = "update"
                # Delete
                else:
    ...
    with ThreadPoolExecutor(max_workers=NUM_THREADS) as executor:
        ...
        while (time.time() - start_time) < duration_sec:
            ...
            if elapsed > 0 and elapsed % 30 == 0:
                print(
                    f"Status: {elapsed}s | Queries: {len(results)} | Rate: {int(len(results)/elapsed)} qps"
                )
            time.sleep(1)
        ...
        for write_ratio, name in scenarios:
            ...
            with open(filename, "w", newline="") as f:
                writer = csv.DictWriter(
                    f, fieldnames=["elapsed_sec", "type", "target", "latency_ms"]
                )
                writer.writeheader()
