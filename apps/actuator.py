...
def run_actuator():
    ...
    for (table, col), total_count in stats.items():
        if total_count >= THRESHOLD:
            ...
            if not cur.fetchone():
                print(
                    f"TREND: {table}.{col} has {total_count} queries in the last {LOOKBACK_MINUTES}m."
                )
                try:
                    cur.execute(
                        f"CREATE INDEX CONCURRENTLY {idx_name} ON {table} ({col})"
                    )
...
if __name__ == "__main__":
    print(
        f"Actuator watching for spikes (Threshold: {THRESHOLD} queries/{LOOKBACK_MINUTES}min)..."
    )
    while True:
        try:
            run_actuator()
        except Exception as e:
            print(f"Actuator waiting: {e}")
        time.sleep(30)
